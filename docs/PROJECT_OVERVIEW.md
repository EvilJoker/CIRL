...
| 能力 | 说明 |
| --- | --- |
| 应用管理 | 统一管理待监控的 App/知识库，支撑后续问答与数据集 |
...
前端提供六大页面（应用管理、问答跟踪、反馈管理、数据集管理、命中分析、效果评估），通过 Sidebar 导航。后端暴露 REST API，负责存储、命中计算与评估计算。
...
详见 `docs/tech/api-reference.md`。

## 6. 前端布局

- **Sidebar 导航**：应用管理 / 问答跟踪 / 反馈管理 / 数据集管理 / 命中分析 / 效果评估
- **App 管理**：列表 + Dialog 创建/编辑
- **问答跟踪**：按应用 + 时间范围筛选、列表展示、录入记录、提交反馈
