...
数据模型定义详见 `docs/design.md` 和 `docs/PROJECT_OVERVIEW.md`，包含：
- `App`
- `QueryRecord`
- `Feedback`
- `Dataset`
...
存储在 SQLite 数据库（`data/cirl.db`）中，支持通过 `DATA_PROVIDER` 环境变量切换到 JSON 文件存储。
...
- **前端**：采用 shadcn-vue + Vue Router，Sidebar 导航八个主要页面（应用管理、QA 跟踪、QA 管理、反馈管理、QA 数据集、Tag 管理、命中分析、效果评估）。
- **文档**：设计总纲、API 参考均已更新为 App 粒度。
