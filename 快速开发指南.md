...
│   ├── dataManager.js        # 数据管理（通过 Provider 抽象层）
│   ├── providers/            # 数据存储 Provider
│   │   ├── baseProvider.js   # Provider 抽象基类
│   │   ├── sqliteProvider.js # SQLite Provider（默认）
│   │   ├── jsonProvider.js   # JSON Provider（备选）
...
2. **检查数据库**
   ...
   # 使用 sqlite3 命令行工具查看（如果已安装）
   sqlite3 data/cirl.db ".tables"
   sqlite3 data/cirl.db "SELECT COUNT(*) FROM apps;"
   ```
...
数据存储在 `data/cirl.db` SQLite 数据库文件中，包含以下表：
- `apps` - 应用列表
- `query_records` - 问答记录
- `feedbacks` - 反馈记录
- `datasets` - 数据集
...
针对写多读少的场景（特别是问答记录），系统实现了写缓冲机制：

- **批量写入**：问答记录先缓存在内存中，达到批次大小（默认 100 条）时批量写入数据库
- **定时刷新**：即使未达到批次大小，也会在指定时间间隔（默认 5 秒）后自动刷新
- **性能提升**：大幅减少数据库 I/O 操作，提升写入性能
- **数据安全**：进程退出时自动刷新所有待写入数据，确保不丢失

可通过环境变量配置：
- `WRITE_BUFFER_BATCH_SIZE`：批次大小（默认 100）
- `WRITE_BUFFER_FLUSH_INTERVAL`：刷新间隔，毫秒（默认 5000）

### JSON Provider（备选）

如需使用 JSON 文件存储，可设置环境变量：
```bash
DATA_PROVIDER=json ./start.sh --local
```
...
environment:
  - NODE_ENV=production
  - PORT=10001
  - DATA_PROVIDER=sqlite  # 默认使用 SQLite，可设置为 json
```
...
数据库文件在 `./data/cirl.db`，升级时无需特殊处理，只需确保：
1. 备份现有数据库文件：`cp data/cirl.db data/cirl.db.backup`
2. 停止旧容器
3. 启动新容器（使用相同的数据目录挂载）

数据库 schema 会在首次启动时自动创建，无需手动迁移。
...
```bash
# 备份数据库文件
cp data/cirl.db data/cirl.db.backup

# 删除损坏的数据库文件，系统会在下次启动时自动重新创建
rm data/cirl.db
```
